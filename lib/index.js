...
app.post("/upload", upload.single("bundle"), (req, res, next) => {
    ...
    console.log("extracting:", req.file.path);
    zip.extractAllToAsync(extractPath, true, function (err) {
        fs.unlink(`.uploads/${req.file.filename}`);
        if (!err) {
            console.log("compiling:", extractPath);
                    ...
                    fs.readFile(configFilePath, "utf8", function (err, data) {
                        if (err) {
                            ...
                            try {
                                ...
                                next(new Error("app.json parse error"));
                                return;
                            }
                            let mainCppPath = `${extractPath}/${appConfig.main}`;
                            // compile main entry file to app.wast
                            childProcess.exec(`gxx -o ${extractPath}/app.wast ${mainCppPath}`, function (err, stdout, stderr) {
                                if (err) {
                                    fs.removeSync(extractPath);
                                    res.send({
                                        status: "error",
                                    ...
                                    childProcess.exec(`gxx -g ${extractPath}/app.abi ${mainCppPath}`, function (err, stdout, stderr) {
                                        if (err) {
                                            res.send({
                                                status: "error",
                                                stdout: `${_stdout}\n${stdout}`,
            ...
            next(new Error("Error when extracting zip file"));
        }
    });
});
...
app.listen(port, () => console.log("GXX Server listening on port ", port));
